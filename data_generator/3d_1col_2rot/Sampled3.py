...
if len(sys.argv) == 2:
    ...
    n = 0
    while os.path.isdir('./data%d' % int(n+1)):
        n += 1
if os.path.isfile('./data%d/3dSprites100000.pov'):
    n += 1
    m = 1
elif len(sys.argv) == 3:
    m = sys.argv[2]
    ...
    m = 1
    while os.path.isfile('./data%d/3dSprites%06d.pov' % (int(n), int(m+1))):
        m += 1
...
print('Creating from folder \'./data%d\' and from file \'3dSprites%06d.pov\'' % (int(n), int(m)))
...
for i in range(m, sample_size+1):
    ...
    # Sample colour
    obj.color = color.hsv_to_rgb(vector(np.random.uniform(),1,1))
